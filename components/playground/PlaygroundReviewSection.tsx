...
export default function PlaygroundReviewSection({ playground }: { playground: Playground }) {
  ...
  }, [summary, content, author, resetForm]);
  ...
  return (
    <div>
      <h3 className="font-bold text-[15px] text-warm-900 mb-3 flex items-center gap-2">
        ...
        <span className="text-xs text-warm-300 font-normal ml-auto">{reviews.length}건</span>
      </h3>
      {reviews.length === 0 && !showForm ? (
        ...
        <div className="flex flex-col gap-2.5">
          {reviews.map((review) => (
            <div key={review.id} className="bg-warm-50 rounded-xl p-3.5">
              ...
              <div className="flex items-center justify-between mt-2.5 pt-2 border-t border-warm-50">
                <div className="flex items-center gap-1.5">
                  <span className="text-[10px] px-1.5 py-0.5 rounded font-semibold bg-blue-50 text-blue-600 border border-blue-100">사용자 리뷰</span>
      ...
      {showForm && (
        <div className="mt-4 bg-warm-50 rounded-2xl p-4 border border-warm-100">
          ...
          <div className="flex flex-col gap-3">
            <div><label htmlFor="pg-review-author" className="block text-xs font-semibold text-warm-500 mb-1">닉네임</label><input id="pg-review-author" type="text" value={author} onChange={(e) => setAuthor(e.target.value)} placeholder="닉네임을 입력하세요" maxLength={30} className="w-full text-sm bg-surface rounded-lg px-3 py-2.5 border border-warm-100 focus:border-primary focus:outline-none transition-colors placeholder:text-warm-300" /></div>
            <div><label htmlFor="pg-review-summary" className="block text-xs font-semibold text-warm-500 mb-1">제목</label><input id="pg-review-summary" type="text" value={summary} onChange={(e) => setSummary(e.target.value)} placeholder="한 줄 요약" maxLength={100} className="w-full text-sm bg-surface rounded-lg px-3 py-2.5 border border-warm-100 focus:border-primary focus:outline-none transition-colors placeholder:text-warm-300" /></div>
            <div><label htmlFor="pg-review-content" className="block text-xs font-semibold text-warm-500 mb-1">내용 <span className="text-warm-300 font-normal ml-1">{content.length}/2000</span></label><textarea id="pg-review-content" value={content} onChange={(e) => setContent(e.target.value)} placeholder="방문 후기를 자유롭게 작성해주세요" maxLength={2000} rows={4} className="w-full text-sm bg-surface rounded-lg px-3 py-2.5 border border-warm-100 focus:border-primary focus:outline-none transition-colors resize-none placeholder:text-warm-300" /></div>
          </div>
