...
function ImageGallery({ place }: { place: Place }) {
  ...
  useEffect(() => { setCurrent(0); setFailedImages(new Set()); }, [place.id]);
          ...
          {hasImages ? (
            images.map((img, i) => (
              <div key={i} className="w-full shrink-0 h-[240px] relative bg-warm-100">
                {failedImages.has(i) ? (
                  <div className={`w-full h-full flex flex-col items-center justify-center bg-gradient-to-br ${preset.gradients[i % preset.gradients.length]}`}>
                    <span className="text-6xl mb-3 drop-shadow-sm">{preset.icons[i % preset.icons.length]}</span>
